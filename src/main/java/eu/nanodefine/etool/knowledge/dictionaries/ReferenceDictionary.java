...
/**
 * Represents and loads the reference dictionary.
 */
public class ReferenceDictionary {
	...
	public ReferenceDictionary(InputStream inputStream) {
		this.log = LoggerFactory.getLogger(ReferenceDictionary.class);

		this.attributeMap = new HashMap<>();

		// Get known attribute and attribute types
		this.knownAttributes = ReflectionUtil
				.getClassConstants(ReferenceAttributes.class);
		this.knownTypes = ReflectionUtil
				.getClassConstants(Attribute.class); // Can be used here
	...
	/**
	 * Second re-implementation of {@link String#trim()}.
	 *
	 * TODO remove…
	 */
	...
	/**
	 * Loads the reference dictionary.
	 */
	private void read() {
		...
		try {
			...
			for (int lineCount = 2; (csvLn = reader
				...
				Boolean validAttr = Boolean.TRUE;
				Boolean validType = Boolean.TRUE;
	...
	public String translateValue(String attributeName, String value) {
		...
		if (ConfigurationUtil.UNKNOWN.equals(value)) {
			result = "Unknown"; // TODO Externalize
		} else {
			...
			if (attribute != null) {
				if (Attribute.TYPE_BINARY.equals(attribute.getType())) {
					...
					String optionSeparator = "<br />"; // TODO Externalize
					List<Option> options = attribute.getOptions(value);
					if (options.isEmpty()) {
						result = "None";
					} else {
						for (Option o : options) {
							result += o.getLabel() + optionSeparator;
						}

						// Remove last separator
						result = result.substring(0,
								result.length() - optionSeparator.length());
					}
